...
vector<int> Parameterization::FindCutVertices(Mesh *mesh, int mode)
{
	Dijkstra *d = new Dijkstra();
	vector<int> poles;
	...
	if(mode == 0)
	{
		...
		poles = FindPoleVertices(mesh);		
	}
	else
	{
		Sampling *fps = new Sampling(4);
		// Find start and end points of the cut.
		vector<int> result = fps->FPS(mesh);
		poles.push_back(result[0]);
		poles.push_back(result[3]);
	}
...
pair<map<int, int>, set<int>> Parameterization::CreateCut(Mesh *mesh, vector<int> &cutVertices, int mode)
{
	// Store global Ids for cut vertices and triangles.
	cutVertices = FindCutVertices(mesh, mode);
